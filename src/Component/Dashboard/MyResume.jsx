...
const MyResume = () => {
  ...
  const handleUpdateResume = () => {
    ...
    const updatedData = {
      templateData: {
        ...
        details: {
          ...
          drivinglicense: selectedResume.templateData?.details?.drivinglicense || '',
          nationality: selectedResume.templateData?.details?.nationality || '',
          placeofbirth: selectedResume.templateData?.details?.placeofbirth || '',
          dateofbirth: selectedResume.templateData?.details?.dateofbirth || '',
          securityclearance: selectedResume.templateData?.details?.securityclearance || '',
          ...
          wantedjobtitle: selectedResume.templateData?.details?.wantedjobtitle || ''
        },
        professionalsummary: selectedResume.templateData?.professionalsummary || '',
    ...
    if (token && selectedResume.id) {
      setIsLoading(true);
      axios.put(`https://api.perfectresume.ca/api/user/resume-update/${selectedResume.id}`, updatedData, {
        headers: {
          Authorization: token
        }
      })
          ...
          <tbody>
            {resumes.map((resume, index) => (
              <tr key={index} className="border-t border-gray-700">
                <td className="py-2 px-4 ">{getFileName(resume.file_path) || "Resume joe"}</td>
